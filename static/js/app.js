...
function wireTabs() {
  document.querySelectorAll(".tab").forEach(el =>
    el.addEventListener("click", () => {
      ...
      // Render-on-activation
      if (el.dataset.tab === "timeline") renderChart(filteredRows());
      if (el.dataset.tab === "table")    renderTable(filteredRows());
      if (el.dataset.tab === "chains")   renderChains(filteredRows());
      if (el.dataset.tab === "insights") renderInsights(filteredRows());
    })
  );

  const fndSel = document.getElementById("fnd");
  if (fndSel) fndSel.innerHTML = '<option value="all">All</option>';

  [
    "fnd","sla","onlyActive","onlyCA","onlyTrans",
    "chainsUseFilters","groupByFoundation","chainsZoom","chainsMaxH",
    "groupHeatByFoundation","insightsView"
  ].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.addEventListener(el.tagName === "INPUT" && el.type === "range" ? "input" : "change", applyFilters);
  });
}
...
function wireButtons() {
  ...
  if ($("btnRunbook")) $("btnRunbook").addEventListener("click", downloadRunbook);
}

// ===== Data loading / watching =====
function manualRefresh() {
  if (CUR_DIR) fetchRows(CUR_DIR);
}

async function loadData() {
  clearError();
  const dir = document.getElementById("watchDir").value.trim();
  if (!dir) { showError("Enter a watch folder path"); return; }
  CUR_DIR = dir;
  await fetchRows(dir);
  // Periodic refresh
  setInterval(() => fetchRows(dir), 30000);
  // Change detection (file signature)
  setInterval(() => checkSig(dir), 10000);
}

async function checkSig(dir) {
  try {
    const r = await safeFetch("/api/version?dir=" + encodeURIComponent(dir));
    const j = await r.json();
    if (j.sig && j.sig !== CUR_SIG) {
      CUR_SIG = j.sig;
      fetchRows(dir);
    }
  } catch (e) { /* silent */ }
...
async function fetchRows(dir) {
  try {
    setStatus("Loading...");
    const r = await safeFetch("/api/rows?dir=" + encodeURIComponent(dir));
    RAW_ROWS = await r.json();
    populateFoundationSelect();
    applyFilters();
    setStatus("Loaded " + RAW_ROWS.length + " rows @ " + new Date().toLocaleTimeString());
  } catch (e) {
...
function populateFoundationSelect() {
  ...
  fndSel.innerHTML = '<option value="all">All</option>' +
    fnds.map(f => `<option value="${f}">${f}</option>`).join("");
...
function filteredRows() {
  ...
  data.sort((a,b) =>
    (a.days_remaining ?? 9e9) - (b.days_remaining ?? 9e9) ||
    a.foundation.localeCompare(b.foundation) ||
    a.cert_name.localeCompare(b.cert_name)
  );
  return data;
}
...
function applyFilters() {
  try {
    ...
    const zv = document.getElementById("chainsZoomVal");
  ...
  if (!chart) {
    chart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: { x: { title: { display: true, text: "Days Remaining" } } }
      }
    });
  }
  return chart;
...
function renderChains(rows) {
  ...
  const groups = {};
  rows.forEach(r => {
    const key = byF ? r.foundation : (r.foundation + "::" + r.cert_name);
  ...
  if (keys.length === 0) {
    ...
    card.className = "card";
    card.innerHTML = "<strong>No chains to display</strong>";
    el.appendChild(card);
    return;
  }
  ...
  keys.forEach(key => {
    ...
    const cert = byF ? "(all certificates)" : rest.join("::");
    const gr = groups[key].slice();
    const byVid = {};
    gr.forEach(x => (byVid[x.version_id] = x));
    ...
    function computeDepth(n, guard = 0) {
      ...
      const base = root ? 1 : computeDepth(p, guard + 1);
      return base + 1;
    }
    ...
    const interLevels = {};
    interCAsAll.forEach(n => {
      const d = n.__depth;
      (interLevels[d] = interLevels[d] || []).push(n);
    });
    ...
    card.innerHTML = `
      <div style="display:flex; align-items:center; gap:12px; margin-bottom:6px; flex-wrap:wrap;">
        <div>
          ...
          <span class="crumbs">ROOT -> TRANSITIONAL -> INTERMEDIATE d1..dn -> LEAF</span>
        </div>
        <span style="margin-left:auto;" class="legend">
          <span class="pill" style="background:#e11d48">&le;30</span>
          <span class="pill" style="background:#f97316">&le;60</span>
          <span class="pill" style="background:#ca8a04">&le;90</span>
...
function resortCard(idx, mode) {
  ...
  function byUrg(a, b) {
    return (a.days_remaining ?? 9e9) - (b.days_remaining ?? 9e9) ||
           (a.version_id_short || "").localeCompare(b.version_id_short || "");
  }
  function byName(a, b) {
    return (a.version_id_short || "").localeCompare(b.version_id_short || "");
  }
  function byDepth(a, b) {
    return (a.__depth || 99) - (b.__depth || 99) || byUrg(a, b);
  }
  ...
  function chipHTML(n) {
    ...
    return `
      <div class="${cls}" title="${ttip.replace(/"/g,'&quot;')}">
  ...
  for (let d = 1; d <= maxInterDepth; d++) {
    buildTier("INTERMEDIATE d" + d, (interLevels[d] || []));
  }
  buildTier("LEAVES", leaves);
}

// expose for inline calls or console
...
function renderInsights(rows) {
  ...
  const events = rows
    .map(r => ({
      date: parseDate(r.valid_until || r.valid_until_raw),
      foundation: r.foundation,
      cert: r.cert_name,
      version: r.version_id_short
    }))
  ...
  if (!events.length) {
    ...
    c.className = "card";
    c.innerHTML = '<div class="muted">No valid expiration dates in scope.</div>';
    root.appendChild(c);
    return;
  }
  ...
  if (!group) appendPanels("Rotation Splash (All Foundations)", events, view, root);
    ...
    const byF = {};
    events.forEach(e => (byF[e.foundation] = byF[e.foundation] || []).push(e));
    Object.keys(byF).sort().forEach(f => appendPanels("Rotation Splash — " + f, byF[f], view, root));
  }
}

function parseDate(s) {
  try { return s ? new Date(s) : null; }
  catch { return null; }
}
...
function buildWeekMap(events) {
  ...
  events.forEach(e => {
    ...
    const k = fmt(wk);
    if (!m.has(k)) m.set(k, { count: 0, items: [] });
    const r = m.get(k);
    r.count++;
    r.items.push(e);
  });
...
function appendPanels(title, events, view, root) {
  if (view === "hist")     return appendWeeklyHistogram(title, events, root);
  if (view === "calendar") return appendCalendarStrip(title, events, root);
  return appendMonthlyPanels(title, events, root);
}
...
function appendCalendarStrip(title, events, root) {
  ...
  const weeks = [];
  for (let d = new Date(minWeek); d <= horizon; d = new Date(d.getTime() + 7*86400000)) weeks.push(new Date(d));

  const counts = weeks.map(w => (weekMap.get(fmt(w)) || { count: 0 }).count);
  const maxCount = Math.max(1, ...counts);

  const cell=14, gap=2, padL=60, padT=18;
  const cols = weeks.length, vbW = padL + cols*(cell+gap) + 10, vbH = padT + 7*(cell+gap) + 18;
  ...
  weeks.forEach((wk,c) => {
    for (let r=0;r<7;r++){
      ...
      svg += `<rect x="${x}" y="${y}" width="${cell}" height="${cell}" fill="${color(val)}"><title>${k}\n${val} expiration(s)</title></rect>`;
    }
  });
  svg += `</svg>`;
  ...
  const card = document.createElement("div");
  card.className = "card";
  card.innerHTML = `<h4 style="margin:0 0 6px 0;">${title}</h4>${svg}`;
...
function appendMonthlyPanels(title, events, root) {
  ...
  const horizon = new Date(); horizon.setMonth(horizon.getMonth()+12);
  const end = new Date(horizon.getFullYear(),horizon.getMonth(),1);
  ...
  wrap.className = "card";
  wrap.innerHTML = `<h4 style="margin:0 0 6px 0;">${title}</h4><div class="ins-grid"></div>`;
  ...
  months.forEach(md => {
    ...
    const sdiff = (s.getDay()+6)%7; // Monday-based
    s.setDate(s.getDate()-sdiff);
    ...
    const cols = weeks.length, vbW = padL + cols*(cell+gap) + 8, vbH = padT + 7*(cell+gap) + 12;
...
function appendWeeklyHistogram(title, events, root) {
  ...
  new Chart(ctx, {
    type: "bar",
    data: { labels: weeks, datasets: [{ label: "Expirations", data, backgroundColor: "#16a34a" }] },
    options: {
      responsive: true,
      plugins: { legend: { display: false } },
...
function appendTopWeeks(title, weekMap, root) {
  ...
  sorted.forEach(([k,rec]) => {
    const list = rec.items.slice(0,5).map(e => `${e.foundation} / ${e.cert} <code>${e.version}</code>`).join("<br/>");
  ...
  top.innerHTML = `
    <h4 style="margin:0 0 6px 0;">Busiest Weeks — ${title}</h4>
...
function exportCSV()  { window.open("/api/export/csv?"  + queryFromFilters(), "_blank"); }
function exportJSON() { window.open("/api/export/json?" + queryFromFilters(), "_blank"); }
function downloadRunbook(){ window.open("/api/runbook?" + queryFromFilters(), "_blank"); }

// Expose specific functions for inline calls (if any)
window.loadData        = loadData;
window.renderInsights  = renderInsights;
window.manualRefresh   = manualRefresh;
window.exportCSV       = exportCSV;
window.exportJSON      = exportJSON;
window.downloadRunbook = downloadRunbook;
